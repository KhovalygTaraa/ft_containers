...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/07/24 13:19:50 by swquinc           #+#    #+#             */
/*   Updated: 2021/07/24 13:19:52 by swquinc          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace ft
{
	...
	public:
		...
		void insert(iterator position, InputIterator first, InputIterator last,
			...
			if (_size + n > _capacity * 2)
				...
				expand_array();
			tmp_position = begin() + insert_pos;
			tmp_first = begin() + first_pos;
			tmp_last = begin() + last_pos - 1;
			if (tmp_first < tmp_position && tmp_last > tmp_position) {
				for(InputIterator it = tmp_last; it >= tmp_position; it--) {
					insert(tmp_position, *(tmp_position + 1 + ft::distance(tmp_position, tmp_last)));
				}
				insert(tmp_position, tmp_first, tmp_position);
				return ;
			}
			if (tmp_last >= tmp_position && tmp_first <= tmp_position) {
				for(InputIterator it = tmp_last; it >= tmp_first; it--)
					tmp_position = insert(tmp_position, *it);
			}
			else {
	...
	void swap(vector<Y, Alloc>& x, vector<Y, Alloc>& y)
	{
		x.swap(y);
	}
